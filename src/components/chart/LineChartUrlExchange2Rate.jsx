...
const LineChartUrlExchange2Rate = () => {
    ...
    priceRateService
      ...
      .then((response) => {
        ...
        response.data.forEach((item) => {
          if (item.seq_no === 1) {
            dataSeq1.push(item.exchange_rate); // seq_no 1에 해당하는 exchange_rate 값
          } else if (item.seq_no === 2) {
            dataSeq2.push(item.exchange_rate); // seq_no 2에 해당하는 exchange_rate 값
          }
        ...
        setChartData({
          labels,
          exchangeRateSeq1: dataSeq1,
          exchangeRateSeq2: dataSeq2,
        }); // 상태에 데이터 저장
      ...
      chartInstance = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "seq_no 1 환율가",
              data: chartData.exchangeRateSeq1, // y축 데이터 (seq_no 1의 exchange_rate)
              ...
              pointHitRadius: 50,
              pointBorderWidth: 2,
              yAxisID: "y", // seq_no 1을 위한 y축
            },
            {
              label: "seq_no 2 환율가",
              data: chartData.exchangeRateSeq2, // y축 데이터 (seq_no 2의 exchange_rate)
...
export default LineChartUrlExchange2Rate;
