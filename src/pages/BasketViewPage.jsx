...
const BasketViewPage = () => {
  ...
  useEffect(() => {
    basketService
      .getBasketList(user_id) // 장바구니 목록 가져오기
      .then((response) => {
        ...
        const fetchAllDetails = response.data.map((item) =>
          basketService.getBasketDetail(user_id, item.goods_gubun)
        );
  ...
  const handleDeleteClick = (seq_no) => {
    ...
    basketService
      ...
      .then((response) => {
        ...
        basketService.getBasketList(user_id).then((response) => {
